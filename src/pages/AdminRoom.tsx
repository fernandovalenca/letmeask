...
import "styles/pages/adminRoom.scss";
...
export function AdminRoom() {
  ...
  const { roomId } = useParams<RoomParamsProps>();
  ...
  async function handleDeleteQuestion(questionId: string) {
    if (window.confirm("Tem certeza que você deseja apagar essa pergunta?")) {
      await database.ref(`rooms/${roomId}/questions/${questionId}`).remove();
    }
  }

  async function handleHighlightQuestion(questionId: string) {
    if (window.confirm("Tem certeza que você deseja destacar essa pergunta?")) {
  ...
  async function handleCheckQuestionAsAnswered(questionId: string) {
    if (
      window.confirm(
        "Tem certeza que você deseja marcar essa pergunta como respondida?"
      )
    ) {
      await database.ref(`rooms/${roomId}/questions/${questionId}`).update({
        isAnswered: true,
